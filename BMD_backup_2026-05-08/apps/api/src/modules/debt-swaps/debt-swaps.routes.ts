...
export async function debtSwapsRoutes(app: FastifyInstance): Promise<void> {
  ...
  /** POST /groups/:groupId/debt-swaps : proposer un swap (Premium / Communauté) */
  app.post("/groups/:groupId/debt-swaps", async (req, reply) => {
    // Spec §3.6 : Swap de dettes = feature Premium uniquement
    await assertFeatureEnabled(req.user.sub, "debtSwap");
    const { groupId } = z
