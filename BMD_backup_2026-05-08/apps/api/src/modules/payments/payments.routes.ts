...
export async function paymentsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.register(async (auth) => {
    ...
    auth.post("/me/checkout-session", async (req) => {
      ...
      const tier = await prisma.planPriceTier.findUnique({
        where: {
          planCode_regionCode: {
      ...
      if (!tier) {
        throw Errors.notFound(
          `Aucun tarif configuré pour ${body.planCode} dans la région ${userRegion.name}.`,
        );
      }
      ...
      if (!stripePriceId) {
        throw Errors.badRequest(
          `Stripe Price ID non configuré pour ${body.planCode} (${body.interval}) en ${userRegion.code}.`,
          {
            tip: "L'admin doit créer le Price dans Stripe Dashboard puis mapper son ID dans la console BMD.",
          },
        );
      }
