...
test.describe("AC-3 · Panneau Réunions", () => {
  test("Le panneau « 🎙️ Réunions » apparaît dans la vue groupe", async ({
    ...
    await page
      .getByPlaceholder(/tontine|voyage|coloc/i)
      .first()
      .fill(`Meetings E2E ${Date.now()}`);
    await page.locator("select").first().selectOption("TONTINE");
    await page.getByRole("button", { name: /créer le groupe/i }).click();
    await page.waitForURL(/\/dashboard\/groups\/[0-9a-f-]{36}/);
