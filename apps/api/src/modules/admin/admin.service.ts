...
// ============================================================
// V95.B — Détection des "users de test" (E2E + seed fixture)
// ============================================================

/**
 * Patterns qui identifient un user créé par les tests automatisés ou
 * par la fixture de seed dev. À exclure des onglets Rentabilité &
 * Activité de la console admin (pour ne pas polluer la vue prod).
 *
 *   - Email se terminant par `@bmd-e2e.local`  (tests Playwright)
 *   - Numéro `+33612360XXX`                    (seed fixture dev)
 */
const TEST_EMAIL_PATTERN = /@bmd-e2e\.local$/i;
const TEST_PHONE_PATTERN = /^\+33612360\d{3}$/;

export function isTestContactValue(value: string): boolean {
  return TEST_EMAIL_PATTERN.test(value) || TEST_PHONE_PATTERN.test(value);
}

/**
 * Retourne l'ensemble des userId qui possèdent au moins un contact
 * matching les patterns de test (E2E ou seed). Une seule requête,
 * exploitable en filtre `NOT IN` ou en post-filter.
 */
export async function getTestUserIds(): Promise<Set<string>> {
  const contacts = await prisma.userContact.findMany({
    where: {
      OR: [
        { value: { endsWith: "@bmd-e2e.local", mode: "insensitive" } },
        { value: { startsWith: "+33612360" } },
      ],
    },
  ...
  for (const c of contacts) {
    if (isTestContactValue(c.value)) ids.add(c.userId);
  }
  return ids;
}
...
export async function recentActivity(limit = 20, excludeTestUsers = false) {
  // V95.B — Exclure les users de test (E2E + seed fixture) sur demande.
  const testIds = excludeTestUsers ? await getTestUserIds() : new Set<string>();
  const userWhere = excludeTestUsers ? { id: { notIn: [...testIds] } } : {};
  const expenseWhere = excludeTestUsers
    ? { paidById: { notIn: [...testIds] } }
    : {};
  // On ne filtre pas les swaps car ils sont attachés à un groupe (pas à un
  // user particulier) ; les groupes sans members test sont déjà rares.
