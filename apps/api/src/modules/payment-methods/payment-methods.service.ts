...
/**
 * V135 — Liste les moyens de paiement d'un AUTRE user, **déchiffrés**, à
 * condition que le caller partage au moins un groupe avec le target.
 *
 * Cas d'usage : tontine. Quand vient le tour d'un membre, les autres doivent
 * pouvoir voir ses coordonnées de paiement (IBAN complet, numéro Wave, email
 * PayPal, …) pour lui envoyer le pot. Sans ça, l'app oblige à demander les
 * coordonnées par WhatsApp/SMS à chaque tour — friction inutile.
 *
 * Modèle de consentement : être membre d'un groupe = consentement implicite
 * à exposer ses moyens de paiement aux autres membres de ce groupe. C'est
 * le contrat social de la tontine traditionnelle (on ne tontinise qu'avec
 * des gens à qui on accepte de partager son IBAN/Wave/…). Si l'utilisateur
 * veut masquer un moyen, il peut le soft-delete.
 *
 * Sécurité :
 *  - Auth obligatoire sur la route (cf. auth hook routes)
 *  - Vérif appartenance groupe commun avant déchiffrement (forbidden sinon)
 *  - Les valeurs en clair restent en mémoire le temps de la sérialisation HTTP
 *  - Aucun log avec la valeur en clair
 */
export async function listPaymentMethodsVisibleToMe(input: {
  callerUserId: string;
  targetUserId: string;
}): Promise<
  Array<{
    ...
    typeEmoji: string;
    label: string;
    /** ⚠️ Valeur en clair — exposée car co-membre du même groupe. */
    value: string;
    last4: string;
    defaultCurrency: string | null;
  }>
> {
  ...
  // Si on appelle sur soi-même, on peut directement retourner — pas besoin
  // de chercher un groupe partagé.
  if (input.callerUserId !== input.targetUserId) {
    // Vérification d'appartenance à un groupe commun : on cherche un GroupMember
    // côté caller dont le groupId existe aussi dans les GroupMember du target.
    ...
    if (!sharedGroup) {
      throw Errors.forbidden(
        "Tu ne partages aucun groupe avec ce membre 🔐",
        {
          tip: "Les moyens de paiement ne sont visibles qu'entre membres d'un même groupe (tontine, coloc, voyage…).",
        },
      );
    }
  }
  ...
  const methods = await prisma.paymentMethod.findMany({
    where: { userId: input.targetUserId, deletedAt: null },
  ...
  return methods.map((m) => {
    ...
    const typeMeta = getTypeLabel(m.type);
    return {
      id: m.id,
      type: m.type,
      typeLabel: typeMeta.name,
