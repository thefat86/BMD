...
// V41 — Refonte mobile dédiée (carte de parrain + network ring).
import { MobileAffiliateView } from "../../../lib/ui/mobile-affiliate-view";
// V164.H5 — Bandeau dynamique des avantages parrain (5 mécaniques A-E configurables admin)
import { ReferralBenefitsBanner } from "../../../lib/ui/referral-benefits-banner";
...
export default function AffiliatePage() {
  ...
  // V41 — Bascule mobile vers vue dédiée (carte parrain Apple Wallet style).
  // Placée APRÈS tous les hooks pour respecter Rules of Hooks.
  if (bpReady && isMobile) {
    return (
      <ResponsiveShell
        breadcrumb={t("affiliate.title") || "Parrainage"}
        mobileTitle={t("affiliate.title") || "Parrainage"}
        back={{ href: "/dashboard" }}
      >
        <MobileAffiliateView />
      </ResponsiveShell>
  ...
  if (!dash.isAffiliate) {
    return (
      ...
      >
        ...
        >
          {/* V52.C2 — SVG gift remplace 🤝 (affiliate = parrainage) */}
          <div style={{ marginBottom: 12, display: "flex", justifyContent: "center" }}>
            <Icon name="gift" size={48} color="var(--saffron)" strokeWidth={1.4} />
          </div>
          <h2
            style={{
  ...
  return (
    ...
    >
      ...
      >
        {/* V164.H5 — Bandeau dynamique des avantages parrain (mois gratuits,
            crédits IA, réduction, points, badges) selon ReferralBenefitConfig.
            Affiché uniquement si au moins une mécanique est activée admin. */}
        <ReferralBenefitsBanner compact={isMobile} />
          ...
          >
            <div className="card-head">
              <h2 style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
                {/* V52.C2 — SVG users remplace 👥 */}
                <Icon name="users" size={20} color="currentColor" strokeWidth={1.6} />
                Mon réseau direct (L1)
              </h2>
              <span
                style={{
                  fontSize: 11,
...
function Kpi({
  ...
  value: string;
  color: string;
  /** Soit un nom d'icône du registry V45 (users, gift, ...), soit un caractère
   *  Unicode mono (⏳) — rétrocompat. */
  icon: string;
}) {
  // V52.C2 — Résout l'icon : si c'est un nom du registry V45 → SVG, sinon
  // glyph Unicode (rétrocompat le temps de migrer toutes les call sites).
  const knownIcon = (
    ["users", "gift", "credit-card", "globe", "bell", "settings"] as const
  ).includes(icon as never);
  return (
    <div
      style={{
        background: "rgba(244,228,193,0.03)",
    ...
    >
      ...
      >
        {knownIcon ? (
          <Icon name={icon as never} size={18} color={color} strokeWidth={1.6} />
        ) : (
          icon
        )}
      </div>
